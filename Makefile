...
ACT_BIND := --bind
ACT_DOCKER_OPTS := -v /var/run/docker.sock:/var/run/docker.sock --privileged
...
install: _check-poetry ## Install all dependencies
...
# Base act command options (reusable)
ACT_BASE_CMD := act workflow_dispatch \
	--container-architecture $(ACT_ARCH) \
	-P $(ACT_PLATFORM) \
	$(ACT_BIND) \
	--container-options "$(ACT_DOCKER_OPTS)"
...
cicd: ## Run CI/CD pipeline (use job=<name>, workflow=<name>, or target=<flatpak|deb|rpm>)
	...
	@if [ -n "$(workflow)" ]; then \
		...
		if [ -n "$(target)" ]; then \
			...
			$(ACT_BASE_CMD) -W "$(WORKFLOWS_DIR)/$(workflow).yml" --matrix target:$(target); \
		else \
			$(ACT_BASE_CMD) -W "$(WORKFLOWS_DIR)/$(workflow).yml"; \
		fi; \
		EXIT_CODE=$$?; \
		...
		if [ -n "$(target)" ]; then \
			echo -e "$(YELLOW)With matrix target: $(target)$(RESET)"; \
			$(ACT_BASE_CMD) --job $(job) --matrix target:$(target); \
		else \
			$(ACT_BASE_CMD) --job $(job); \
		fi; \
		EXIT_CODE=$$?; \
