...
def test_config_degiro_status():
    ...
    with patch('stonks_overwatch.services.degiro.degiro_service.DeGiroService.check_connection', return_value=True):
        assert config.is_degiro_connected()
        assert config.is_degiro_enabled_and_connected()

    with patch('stonks_overwatch.services.degiro.degiro_service.DeGiroService.check_connection', return_value=False):
