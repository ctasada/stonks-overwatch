...
class TestAssetLogoView(TestCase):
    ...
    def test_get_country_logo(self, mock_get):
        ...
        self.assertEqual(response["Content-Type"], "image/svg+xml")
        mock_get.assert_called_once_with(
            "https://cdnjs.cloudflare.com/ajax/libs/twemoji/17.0.2/svg/1f1fa-1f1f8.svg", timeout=5
        )
    ...
    def test_get_sector_logo(self, mock_get):
        ...
        self.assertEqual(response["Content-Type"], "image/svg+xml")
        mock_get.assert_called_once_with(
            "https://cdnjs.cloudflare.com/ajax/libs/twemoji/17.0.2/svg/1f5a5-fe0f.svg", timeout=5
        )
    ...
    def test_emoji_to_svg(self):
        ...
        self.assertEqual(url, "https://cdnjs.cloudflare.com/ajax/libs/twemoji/17.0.2/svg/1f1fa-1f1f8.svg")

        # Test with a sector emoji (🖥️)
        emoji = "🖥️"
        url = self.view._AssetLogoView__emoji_to_svg(emoji)
        self.assertEqual(url, "https://cdnjs.cloudflare.com/ajax/libs/twemoji/17.0.2/svg/1f5a5-fe0f.svg")
